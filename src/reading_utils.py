...
def read_data(file_path):
    ...
    separated by whitespace. Each row is read and split into two lists based on these columns.
...
def read_and_prepare_data(file_path):
    ...
    This function uses `read_data` to fetch data from the specified file and then converts 
    the data into numpy arrays for further processing.
...
def read_config(input_file):
    ...
    The function expects a configuration file in INI format with sections for 'ModelParameters' 
    and 'SamplingParameters'. It extracts parameters for the model and sampling process, providing
    fallback values if specific entries are missing.
